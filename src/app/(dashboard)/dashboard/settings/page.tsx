...
function ProfileSection({ profile, user, userId, organization, onRefresh }: { profile: any, user: any, userId: string | null, organization?: any, onRefresh: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t('settings.profile.updated' as any));
      onRefresh();
    } catch (error) {
      console.error(error);
      toast.error(t('settings.profile.saveError' as any));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t('settings.profile.photoUpdated' as any));
      onRefresh();
    } catch (error) {
      console.error(error);
      toast.error(t('settings.profile.photoError' as any));
      setPreviewUrl(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.profile.title' as any)}</h3>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex-1 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label={t('settings.profile.fullName' as any)} value={formData.full_name} onChange={(v) => setFormData({ ...formData, full_name: v })} />
            ...
            <Field label={t('settings.profile.position' as any)} value={formData.cargo} onChange={(v) => setFormData({ ...formData, cargo: v })} />
            <Field label={t('settings.profile.company' as any)} value={formData.company} onChange={(v) => setFormData({ ...formData, company: v })} />
            <Field label={t('settings.profile.phone' as any)} value={formData.phone} placeholder="+34..." onChange={(v) => setFormData({ ...formData, phone: v })} />
          </div>
          <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-[#1E3A5F]">
            <button onClick={handleSave} disabled={loading} className="w-full sm:w-auto px-6 py-2.5 bg-[#1B4FD8] hover:bg-blue-700 text-white font-semibold text-[14px] rounded-xl transition-all shadow-lg active:scale-[0.98] uppercase tracking-tight flex items-center justify-center gap-2">
              ...
              {t('settings.profile.save' as any)}
            </button>
          </div>
        </div>
      </div>
...
function OrganizationSection({ organization, onRefresh, userId, user }: { organization: any, onRefresh: () => void, userId: string | null, user: any }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === '42703') {
          toast.error(t('settings.org.sqlError' as any));
          return;
        }
        throw error;
      }
      ...
      console.error(error);
      toast.error(t('settings.org.saveError' as any));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.org.title' as any)}</h3>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label={t('settings.org.businessName' as any)} value={formData.name} onChange={(v) => setFormData({ ...formData, name: v })} />
        <Field label={t('settings.org.businessActivity' as any)} value={formData.sector} onChange={(v) => setFormData({ ...formData, sector: v })} />
...
function WorkingHoursSection({ organization, onRefresh, userId, user }: { organization: any, onRefresh: () => void, userId: string | null, user: any }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error(t('settings.hours.saveError' as any));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.hours.title' as any)}</h3>
      </div>

      <div className="space-y-3">
        <div className="space-y-2">
          <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.hours.workingDays' as any)}</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.hours.open' as any)}</label>
            <select
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.hours.close' as any)}</label>
            <select
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.org.paymentMethods' as any)}</h3>
          <p className="text-[12px] text-[#64748B] dark:text-[#94A3B8]">{t('settings.org.paymentMethodsDesc' as any)}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="p-4 bg-slate-50 dark:bg-[#111F3A] rounded-xl border border-slate-100 dark:border-[#1E3A5F] space-y-3">
          ...
          {bizumEnabled && (
            <Field label={t('settings.org.bizumNumber' as any)} value={bizumNumber} placeholder="600 000 000" onChange={setBizumNumber} />
          )}
        </div>
        ...
        <div className="p-4 bg-slate-50 dark:bg-[#111F3A] rounded-xl border border-slate-100 dark:border-[#1E3A5F] space-y-3">
          ...
          {ibanEnabled && (
            <Field label={t('settings.org.iban' as any)} value={ibanNumber} placeholder="ES00 0000 0000 0000 0000 0000" onChange={setIbanNumber} />
          )}
        </div>
      </div>
...
function EmailCorporativoSection({ organization }: { organization: any }) {
  ...
  const handleSave = async () => {
    ...
    if (!formData.alert_email.trim()) {
      toast.error(t('settings.email.alertEmailRequired' as any));
      return;
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(formData.alert_email.trim())) {
      toast.error(t('settings.email.alertEmailInvalid' as any));
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      toast.success(t('settings.email.saved' as any));
    } catch (error) {
      console.error(error);
      toast.error(t('settings.email.saveError' as any));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.email.title' as any)}</h3>
      </div>
      ...
      <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl">
        <h4 className="flex items-center gap-2 text-[12px] font-bold text-blue-700 dark:text-blue-400 mb-2 uppercase tracking-tight">
          <Info className="w-4 h-4" /> {t('settings.email.connection' as any)}
        </h4>
        <div className="space-y-2 text-[11px] text-blue-800/80 dark:text-blue-300/80 leading-relaxed font-medium">
          <p>{t('settings.email.forwardingTo' as any)}: <span className="font-bold text-blue-600 dark:text-blue-400 select-all underline">tu_usuario@sffalcon.com</span></p>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-3">
        <Field label={t('settings.email.businessEmail' as any)} value={formData.email_inbound} placeholder="info@tunegocio.com" onChange={(v) => setFormData({ ...formData, email_inbound: v })} />
        <Field label={t('settings.email.displayName' as any)} value={formData.email_display_name} placeholder="Mi Negocio" onChange={(v) => setFormData({ ...formData, email_display_name: v })} />
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-[#64748B] dark:text-[#94A3B8] uppercase tracking-[0.08em] ml-1">{t('settings.email.signature' as any)}</label>
          <textarea
            value={formData.email_signature}
            onChange={(e) => setFormData({ ...formData, email_signature: e.target.value })}
            placeholder={t('settings.email.signaturePlaceholder' as any)}
        ...
        <ToggleCard title={t('settings.email.replyWithAI' as any)} subtitle={t('settings.email.replyWithAISub' as any)} active={formData.email_ai_enabled} onChange={(v) => setFormData({ ...formData, email_ai_enabled: v })} />
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2 ml-1">
            <label className="text-[11px] font-medium text-[#64748B] dark:text-[#94A3B8] uppercase tracking-[0.08em]">
              {t('settings.email.alertEmail' as any)} <span className="text-red-500">*</span>
            </label>
            <span className="text-[10px] text-[#64748B] dark:text-[#475569]">({t('settings.email.humanAssistance' as any)})</span>
          </div>
          <input
            type="email"
...
function AutonomousAISection({ organization, userId, user, onRefresh }: { organization: any, userId: string | null, user: any, onRefresh: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error(t('settings.ai.saveError' as any));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.ai.title' as any)}</h3>
        </div>
        {!isPro && (
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-1 gap-3">
          <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.ai.features' as any)}</label>
          <ToggleCard
            title={t('settings.ai.autoAppointments' as any)}
          ...
          />
          <ToggleCard
            title={t('settings.ai.autoClients' as any)}
        ...
        <div className={cn("space-y-3", !isPro && "opacity-50 pointer-events-none")}>
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.ai.personality' as any)}</label>
            <div className="flex gap-2 p-1 bg-slate-50 dark:bg-[#111F3A] rounded-xl w-fit">
              {['amigable', 'profesional'].map(p => (
                ...
                >
                  {p === 'amigable' ? t('settings.ai.personalities.friendly' as any) : t('settings.ai.personalities.professional' as any)}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('settings.ai.businessDesc' as any)}</label>
            <textarea
              ...
              placeholder={t('settings.ai.businessDescPlaceholder' as any)}
            />
          </div>
        </div>
...
function SubscriptionSection({ organization, userId, user, onRefresh }: { organization: any, userId: string | null, user: any, onRefresh: () => void }) {
  ...
  const handleCancelSubscription = async () => {
    if (!confirm(t('settings.subscription.cancelConfirm' as any))) return;
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || t('settings.subscription.cancelError' as any));
      }
    } catch (error) {
      toast.error(t('settings.subscription.connectionError' as any));
    } finally {
      setCancelling(false);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        <CreditCard className="w-5 h-5 text-[#1B4FD8]" />
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.subscription.title' as any)}</h3>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.subscription.planTitle' as any)}</h4>
              <span className="px-2 py-0.5 rounded-full text-white text-[9px] font-black bg-indigo-600 uppercase">PRO</span>
            </div>
            {daysLeft > 0 ? (
              <p className="text-[11px] text-[#64748B] dark:text-[#94A3B8]">
                {t('settings.subscription.trial' as any)}: <span className="font-bold text-[#1B4FD8]">{daysLeft} {t('settings.subscription.days' as any)}</span>
              </p>
            ) : (
              <p className="text-[11px] text-[#64748B] dark:text-[#94A3B8]">{t('settings.subscription.active' as any)}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {daysLeft > 0 ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {cancelling ? "..." : t('settings.subscription.cancel' as any)}
              </button>
              <button
                onClick={() => toast.success(t('common.loading' as any))}
                className="py-2 bg-[#1B4FD8] text-white font-bold text-[11px] rounded-xl transition-all shadow-lg uppercase"
              >
                {t('settings.subscription.invoices' as any)}
              </button>
            </div>
          )}
        </div>
...
function UpgradeModalInternal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!priceId) {
        toast.error(t('settings.upgrade.priceError' as any))
        return
      }

      if (!organization?.id) {
        toast.error(t('settings.upgrade.orgIdError' as any))
        return
      }
      ...
      if (data.url) {
        ...
        toast.error(data.error || t('settings.upgrade.checkoutError' as any))
      }
    } catch (error) {
      console.error(error)
      toast.error(t('settings.subscription.connectionError' as any))
  ...
  const features = [
    t('settings.upgrade.features.crm' as any),
    t('settings.upgrade.features.comms' as any),
    t('settings.upgrade.features.ai' as any),
    t('settings.upgrade.features.finances' as any),
    t('settings.upgrade.features.products' as any),
    t('settings.upgrade.features.metrics' as any),
    t('settings.upgrade.features.team' as any),
    t('settings.upgrade.features.aiPanel' as any)
  ]
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="px-6 pt-6 pb-4 border-b border-slate-100 dark:border-[#1E3A5F] flex items-center justify-between">
              <h2 className="text-[17px] font-semibold text-[#0F172A] dark:text-[#F1F5F9] tracking-tight">{t('settings.upgrade.title' as any)}</h2>
            ...
            <div className="p-6 space-y-5">
              <div className="text-center">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full mb-4">
                  ...
                  <span className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em]">{t('settings.upgrade.freeTrial' as any)}</span>
                </div>
                <h2 className="text-xl font-black text-[#0F172A] dark:text-[#F1F5F9] mb-2 uppercase tracking-tighter">{t('settings.upgrade.planName' as any)}</h2>
                <p className="text-slate-500 dark:text-slate-400 text-[11px] font-bold uppercase tracking-widest">{t('settings.upgrade.planSlogan' as any)}</p>
              </div>
...
function WhatsAppSection({ organization }: { organization: any }) {
  ...
  const handleCopy = () => {
    ...
    toast.success(t('settings.whatsapp.linkCopied' as any));
  };
  ...
  return (
    <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-[12px] p-5 shadow-sm transition-colors">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[16px] font-semibold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.whatsapp.title' as any)}</h3>
      </div>

      <div className="space-y-4">
        <div className="p-4 bg-slate-50 dark:bg-[#111F3A] rounded-xl border border-slate-100 dark:border-[#1E3A5F] space-y-2">
          <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.whatsapp.optionA' as any)}</h4>
          <p className="text-[12px] text-[#64748B] dark:text-[#94A3B8] leading-relaxed">
            {t('settings.whatsapp.optionADesc' as any)}
          </p>
          <div className="flex items-center gap-2 mt-2">
            ...
            >
              {t('settings.whatsapp.copy' as any)}
            </button>
          </div>
        </div>

        <div className="p-4 bg-slate-50 dark:bg-[#111F3A] rounded-xl border border-slate-100 dark:border-[#1E3A5F] space-y-2">
          <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.whatsapp.optionB' as any)}</h4>
          <p className="text-[12px] text-[#64748B] dark:text-[#94A3B8] leading-relaxed">
            {t('settings.whatsapp.optionBDesc' as any)}
          </p>
        </div>

        <div className="p-4 bg-slate-50 dark:bg-[#111F3A] rounded-xl border border-slate-100 dark:border-[#1E3A5F] space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="text-[14px] font-bold text-[#0F172A] dark:text-[#F1F5F9]">{t('settings.whatsapp.optionC' as any)}</h4>
            <span className="px-2.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-[10px] font-bold rounded-full uppercase tracking-wider">{t('settings.whatsapp.recommended' as any)}</span>
          </div>
          <p className="text-[12px] text-[#64748B] dark:text-[#94A3B8] leading-relaxed">
            {t('settings.whatsapp.optionCDesc' as any)}
          </p>
        </div>
      </div>
    </div>
