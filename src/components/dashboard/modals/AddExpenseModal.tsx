...
import { getSectorCategories } from '@/lib/financeCategories';
...
// Base categories removed in favor of dynamic universal ones below
...
const DEFAULT_TYPE_MAP: Record<string, string> = {
  ...
  'INTERNET': 'suscripcion',
};

// No longer used statically
...
export default function AddExpenseModal({ isOpen, onClose, onSuccess }: AddExpenseModalProps) {
  ...
  const { expense: universalCategories } = getSectorCategories();
  const defaultSuggestions = [...categoriasGenericas, ...universalCategories.map(c => c.name)];

  const allSuggestions = userSuggestions.length > 0 ? userSuggestions : defaultSuggestions;

  const filteredSuggestions = allSuggestions.filter((s) =>
    s.toUpperCase().includes(concept.toUpperCase())
  );
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      if (!error && data) {
        const typedData = data as { concept: string, type: string }[];
        const concepts = Array.from(new Set(typedData.map(d => d.concept.toUpperCase())));
        const types: Record<string, string> = {};
        typedData.forEach(d => {
          if (!types[d.concept.toUpperCase()]) {
            types[d.concept.toUpperCase()] = d.type;
          }
        });
        setUserSuggestions(concepts);
        setUserTypeMap(types);
      }
    };
  ...
  const handleSelectSuggestion = (suggestion: string) => {
    ...
    setShowSuggestions(false);
    const autoType = userTypeMap[suggestion.toUpperCase()] || DEFAULT_TYPE_MAP[suggestion.toUpperCase()];
    if (autoType) {
      setType(autoType);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-3 md:space-y-3.5 lg:space-y-4">
          ...
          <div>
            ...
            <div className="relative" ref={suggestionsRef}>
              ...
              />
              {showSuggestions && concept && filteredSuggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-xl shadow-2xl z-50 max-h-56 overflow-y-auto ring-1 ring-black/5 dark:ring-white/5 backdrop-blur-sm">
            ...
            {errors.concept && (
              <p className="text-[10px] md:text-[11px] lg:text-[12px] text-[#EF4444] mt-1">{errors.concept}</p>
            )}
          </div>
          ...
          <div>
            ...
            >
              {expenseTypes.map((et) => (
                <option key={et.value} value={et.value} className="bg-[#111F3A] text-white">
                  {et.label}
                </option>
              ))}
            </select>
          ...
          <div className="flex gap-3 pt-6 border-t border-slate-100 dark:border-[#1E3A5F]">
            <button
              type="button"
              onClick={onClose}
              disabled={isLoading}
