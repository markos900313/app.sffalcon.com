...
export default function AddBusinessIncomeModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const isAutonomo = organization?.sector_config?.tipo === 'autonomo';
  const isRestauracion = organization?.sector_config?.grupo === 'P1_restauracion' || organization?.sector_config?.grupo === '4_hosteleria';
  const isOcio = organization?.sector_config?.grupo === '6_ocio';

  const incomeCategories = INCOME_CATEGORIES_G1;

  const [concept, setConcept] = useState(incomeCategories[0]);
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      setConcept(''); setAmount(''); setClient(''); setProject(''); setNotes('');
      ...
      <div className="bg-white dark:bg-[#111F3A] border border-[#E2E8F0] dark:border-[#1E3A5F] rounded-xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[11px] text-[#64748B] dark:text-[#94A3B8] mt-0.5 uppercase tracking-wide">Cobro o Factura</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <select
              ...
              className={inputCls + (errors.concept ? ' border-red-400' : '') + " bg-[#111F3A] text-white"}
            >
          ...
          {/* Ocultar Cliente y Proyecto para Grupo 1 en finanzas rápidas si se prefiere, 
              pero el usuario no lo pidió explícitamente aquí. Se mantienen por ahora. */}
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[11px] font-medium text-[#64748B] uppercase tracking-wider mb-1.5">Contacto</label>
              <input type="text" value={client} onChange={e => setClient(e.target.value)} placeholder="Nombre contacto" className={inputCls} />
            </div>
            <div>
              <label className="block text-[11px] font-medium text-[#64748B] uppercase tracking-wider mb-1.5">Proyecto / Entidad</label>
