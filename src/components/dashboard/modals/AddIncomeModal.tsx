...
export default function AddIncomeModal({ isOpen, onClose, onSuccess }: AddIncomeModalProps) {
  ...
  const { income: universalCategories } = getSectorCategories();
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      if (!error && data) {
        const unique = Array.from(new Set((data as { concept: string }[]).map(d => d.concept.toUpperCase())));
