...
const tabs = ["Todos", "Correos", "WhatsApp", "Respondidos", "Pendientes"];
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin dark:scrollbar-thumb-[#1E3A5F] dark:scrollbar-track-transparent">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center opacity-60">
            ...
            <p className="text-sm font-medium text-[#64748B] dark:text-[#94A3B8]">No hay conversaciones</p>
            <p className="text-[11px] text-[#94A3B8] dark:text-[#475569]">Los mensajes aparecerán aquí</p>
          </div>
        ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 md:gap-2">
                  <span className={cn(
                    ...
                    {chat.status === 'pending' || chat.status === 'seen' ? 'PENDIENTE' :
                      chat.status === 'responded' ? 'RESPONDIDO' : 'RESUELTO'}
                  </span>
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#111F3A] w-full max-w-sm rounded-2xl p-6 shadow-2xl border border-slate-200 dark:border-[#1E3A5F] animate-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 mb-4 text-red-500">
          ...
          <h3 className="text-lg font-bold text-[#0F172A] dark:text-[#F1F5F9]">¿Eliminar conversación?</h3>
        </div>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
          Esta acción no se puede deshacer y borrará permanentemente todos los mensajes asociados.
        </p>
